use std::error::Error;
use reqwest::Client;
use super::models::{DeviceCodeResponse, CLIENT_ID, MicrosoftToken};
...
pub async fn poll_for_ms_token(device_code: &str, interval: u64) -> Result<MicrosoftToken, AnyError> {
    let client = Client::new();
    loop {
        let res = client
        ...
        if let Some(token) = body.get("access_token") {
            return Ok(MicrosoftToken {
                ...
                refresh_token: body.get("refresh_token").map(|v| v.as_str().unwrap().to_string()),
            });
        }
...
pub async fn get_xbox_token(ms_token: &str) -> Result<(String, String), AnyError> {
    ...
    let res = client.post("https://user.auth.xboxlive.com/user/authenticate").json(&body).send().await?;
    let status = res.status();
    let text = res.text().await?;
...
pub async fn get_xsts_token(xbox_token: &str) -> Result<String, AnyError> {
    ...
    let res = client.post("https://xsts.auth.xboxlive.com/xsts/authorize").json(&body).send().await?;
    let status = res.status();
    let text = res.text().await?;
...
pub async fn get_minecraft_token(xsts_token: &str, uhs: &str) -> Result<String, AnyError> {
    ...
    let res = client.post("https://api.minecraftservices.com/authentication/login_with_xbox").json(&body).send().await?;
    let status = res.status();
    let text = res.text().await?;
    ...
    let val: serde_json::Value = serde_json::from_str(&text)?;
    Ok(val["access_token"].as_str().ok_or("No access_token in response")?.to_string())
}
